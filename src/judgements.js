...
UglifyJS.AST_Constant.prototype.check = function(gamma, dynamics) {
...
UglifyJS.AST_Symbol.prototype.check = function(gamma, dynamics, doNotWrap) {
	...
	if (T === null || T === undefined) {
		...
		if (!dynamic) gamma.push(new Classes.TypeEnvEntry(this.name, this, T));
	}

	if (dynamic) {
		W.push(new Classes.Wrapper(this, this.parent(), T));
	}
	...
	switch(this.operator) {
		case ("="):
			...
			if (! (this.left instanceof UglifyJS.AST_Dot)) {
				...
				if (!dynamicWrite) C.push(new Classes.LEqConstraint(j2.T, j1.T, this.right));
