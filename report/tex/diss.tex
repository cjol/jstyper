...
state of computation is described by a configuration triple of the form
...
references share a syntactic similarity to dereference-able expressions, they are
distinct and do not reduce down to the value itself. This is important, else we
could end up with assignments of the form \js{5 = 6;}.
...
particular variable has the same type. I thus use the same
...
$\mathtt{[\![\ \ ]\!]}$ will never appear in the
original program, and so the type judgements for these kinds of expression
...
My implementation generates constraints by precisely following the rules of the
formally specified type judgements. UglifyJS parses the input code and creates
an abstract syntax tree. Each node in the tree inherits from a base prototype
object representing the node type.  For example, the expression \js{x.l = 5} is
represented by an \js{AST_Assign} node containing an \js{AST_Dot} and
an \js{AST_Number} node.  I added a \js{check(gamma)} method to the
...
\begin{proof}
  The proofs of Theorems~\ref{expProgress} and~\ref{mProgress} are by rule induction over the type judgements.
  Take
  \begin{multline*}
  	\indHypTwo \eqdef \\
...
\begin{figure}
  ...
  \subfloat[The resulting Jasmine test failure]{
...
implement before the end of the project, leaving me with relatively few to
test. The remaining tests, however, required very little modification in order
...
Since the SunSpider benchmarks are written as though statically typed, and are
well tested to eliminate errors, there were no occasions for dynamic type
errors to be thrown. With the \js{Math} object and shift operators wrapped as
described above, the correct results were computed, and so I am confident that
my gradual typing compiler is sound.
...
typing arrays, and of supporting the dynamic growth of objects. In carrying out the
project, I have learned to use a range of new tools, from Ott to memwatch,
...
able to type check a much larger set of existing JavaScript programs. One of the main
...
analogous to fitting a square peg in a round hole, in practice the addition of
gradual typing wrappers has made my approach successful. The additional safety
guarantees which can be offered about such statically typed programs are
valuable, and future integration within code intelligence tools could offer
even greater benefits to JavaScript developers.  The process has been
instructive from start to finish, and I hope to pursue the ideas formed here
further in future.
